...
// cookie 名（同时用于 SSR + 客户端使用）
const COOKIE_NAME = 'ywty.auth'

// 通过 Nuxt 内置 useCookie 实现 SSR + 客户端双端同步
// 注意：useCookie 默认会用 JSON.stringify + encodeURIComponent 编码
...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    // 通用 hydrate：SSR 和客户端都从 cookie 读取
    hydrate() {
    ...
    setTokens(payload: TokenPair) {
      ...
      // 通过 Nuxt useCookie 同步到两端
      // useCookie 写入时会做 JSON.stringify + encodeURIComponent 一次，
